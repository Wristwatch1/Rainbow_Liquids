...
public class BlockLightGreySpectra extends BlockFluidClassic 
{
    ...
    public BlockLightGreySpectra(Fluid fluid, Material material) {
            ...
            setLightLevel(0.9f);
            try {
              coloredlightscore.src.api.CLApi.setBlockColorRGB(this, 10, 10, 10);
            } catch (Throwable e) {
    ...
    public void registerBlockIcons(IIconRegister iconregister) {
            this.stillIcon = iconregister.registerIcon("wristwatch1_rainbowwater:BlockLightGreySpectraStill");
            this.flowingIcon = iconregister.registerIcon("wristwatch1_rainbowwater:BlockLightGreySpectraFlowing");
    }
    @Override
	...
	public void onEntityCollidedWithBlock(World world, int i, int j, int k, Entity entity)//when an entity steps into the liquid
	{
		...
		{         
			((EntityPlayer) entity).addPotionEffect(new PotionEffect(Potion.resistance.getId(),1200, 1));//allows the potion affect to effect players
			}  
		if (entity instanceof EntityLiving) 
		{         
			((EntityLiving) entity).addPotionEffect(new PotionEffect(Potion.resistance.getId(),1200, 1));//allows the potion affect to effect mobs
			}  
	}
